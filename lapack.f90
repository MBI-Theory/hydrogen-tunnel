...
!
  ...
  character(len=clen), save :: rcsid_lapack = "$Id: lapack.f90,v 1.10 2021/04/26 15:44:44 ps Exp $"
...
!*lq module procedure lapack_quad_zgelss
  ...
  interface lapack_stev
     ...
     module procedure lapack_sstev_novectors
     module procedure lapack_dstev_novectors
...
!*lq end subroutine lapack_quad_dgelss
  ...
  subroutine lapack_sstev_novectors(d,e)
    real(srk), intent(inout) :: d(:)   ! In:  Diagonal elements of the matrix 
                                       ! Out: Eigenvalues, ascending order
    real(srk), intent(inout) :: e(:)   ! In:  Sub-/super-diagonal elements of the matrix
                                       ! Out: Destroyed
    ...
    real(srk) :: work(max(1,2*size(d)-2))
    integer   :: info      ! Must be of default integer kind
    real(srk) :: z(1,1)    ! Dummy array, not accessed
    
    call sstev('N',size(d),d,e,z,1,work,info)

    if (info/=0) then
      write (out,"(' dstev returned ',i8)") info
      stop 'lapack_sstev_novectors - sstev failed'
    end if
  end subroutine lapack_sstev_novectors

  subroutine lapack_dstev_novectors(d,e)
    real(drk), intent(inout) :: d(:)   ! In:  Diagonal elements of the matrix 
                                       ! Out: Eigenvalues, ascending order
    real(drk), intent(inout) :: e(:)   ! In:  Sub-/super-diagonal elements of the matrix
                                       ! Out: Destroyed
    ...
    real(drk) :: work(max(1,2*size(d)-2))
    integer   :: info      ! Must be of default integer kind
    real(drk) :: z(1,1)    ! Dummy array, not accessed
    
    call dstev('N',size(d),d,e,z,1,work,info)

    if (info/=0) then
      write (out,"(' dstev returned ',i8)") info
      stop 'lapack_dstev_novectors - dstev failed'
    end if
  end subroutine lapack_dstev_novectors
...
!*lq end subroutine lapack_quad_zgeev2
  ...
  !
  !  Solves generalized eigenproblem:
  !
  !   beta(i) A C(i) = alpha(i) B C(i)
  !
  subroutine lapack_zggev(a,b,alpha,beta)
    complex(kind=drk), intent(inout) :: a(:,:)   ! In:  Matrix to be diagonalized
                                                 ! Out: Left eigenvectors
    complex(kind=drk), intent(inout) :: b(:,:)   ! In:  Matrix to be diagonalized
                                                 ! Out: Right eigenvectors
    complex(kind=drk), intent(out)   :: alpha(:) ! Out: Eigenvalues
    complex(kind=drk), intent(out)   :: beta (:) ! Out: Eigenvalues

    integer(ik)               :: n, alloc, lwork, info
    complex(drk), allocatable :: vl(:,:), vr(:,:), work(:)
    real(drk), allocatable    :: rwork(:)
    complex(drk)              :: awork(1)

    n = size(a,dim=1)
    if (size(a,dim=2)/=n .or. size(b,dim=1)/=n .or. size(b,dim=2)/=n .or. size(alpha)/=n .or. size(beta)/=n) then
      stop 'lapack_zggev - bad array dimensions'
    end if
    allocate (vl(n,n),vr(n,n),rwork(8*n),stat=alloc)
    if (alloc/=0) then
      stop 'lapack_zggev - allocate failed (1)'
    end if

    lwork = -1 
    call zggev('V','V',n,a,n,b,n,alpha,beta,vl,n,vr,n,awork,lwork,rwork,info)
    if (info/=0) then
      write (out,"(' zggev workspace query returned ',i8)") info
      stop 'lapack_zggev - failed (1)'
    end if
    lwork = nint(real(awork(1),kind=rk))
    allocate (work(lwork),stat=alloc)
    if (alloc/=0) then
      stop 'lapack_zggev - allocate failed (2)'
    end if

    call zggev('V','V',n,a,n,b,n,alpha,beta,vl,n,vr,n,work,lwork,rwork,info)
    if (info/=0) then
      write (out,"(' zggev returned ',i8)") info
      stop 'lapack_zggev - failed (2)'
    end if
...
!*lq subroutine lapack_quad_zggev(a,b,alpha,beta)
!*lq   complex(kind=qrk), intent(inout) :: a(:,:)   ! In:  Matrix to be diagonalized
!*lq                                                ! Out: Left eigenvectors
!*lq   complex(kind=qrk), intent(inout) :: b(:,:)   ! In:  Matrix to be diagonalized
!*lq                                                ! Out: Right eigenvectors
!*lq   complex(kind=qrk), intent(out)   :: alpha(:) ! Out: Eigenvalues
!*lq   complex(kind=qrk), intent(out)   :: beta (:) ! Out: Eigenvalues

!*lq   integer(ik)               :: n, alloc, lwork, info
!*lq   complex(qrk), allocatable :: vl(:,:), vr(:,:), work(:)
!*lq   real(qrk), allocatable    :: rwork(:)
!*lq   complex(qrk)              :: awork(1)

!*lq   n = size(a,dim=1)
!*lq   if (size(a,dim=2)/=n .or. size(b,dim=1)/=n .or. size(b,dim=2)/=n .or. size(alpha)/=n .or. size(beta)/=n) then
!*lq     stop 'lapack_quad_zggev - bad array dimensions'
!*lq   end if
!*lq   allocate (vl(n,n),vr(n,n),rwork(8*n),stat=alloc)
!*lq   if (alloc/=0) then
!*lq     stop 'lapack_quad_zggev - allocate failed (1)'
!*lq   end if

!*lq   lwork = -1 
!*lq   call quad_zggev('V','V',n,a,n,b,n,alpha,beta,vl,n,vr,n,awork,lwork,rwork,info)
!*lq   if (info/=0) then
!*lq     write (out,"(' quad_zggev workspace query returned ',i8)") info
!*lq     stop 'lapack_quad_zggev - failed (1)'
!*lq   end if
!*lq   lwork = nint(real(awork(1),kind=rk))
!*lq   allocate (work(lwork),stat=alloc)
!*lq   if (alloc/=0) then
!*lq     stop 'lapack_quad_zggev - allocate failed (2)'
!*lq   end if

!*lq   call quad_zggev('V','V',n,a,n,b,n,alpha,beta,vl,n,vr,n,work,lwork,rwork,info)
!*lq   if (info/=0) then
!*lq     write (out,"(' quad_zggev returned ',i8)") info
!*lq     stop 'lapack_quad_zggev - failed (2)'
!*lq   end if

!*lq   a = vl
!*lq   b = vr

!*lq   deallocate (vl,vr,work,rwork)
!*lq end subroutine lapack_quad_zggev
...
!*lq  subroutine lapack_quad_zheev(h,e)
!*lq    complex(qrk), intent(inout) :: h(:,:)  ! In:  Hermitian matrix to be diagonalized
!*lq                                           ! Out: Eigenvectors
!*lq    real(qrk), intent(out)   :: e(:)       ! Out: Eigenvalues
!*lq
!*lq    complex(qrk) :: work(50*size(h,dim=1))
!*lq    real(qrk)    :: rwork(3*size(h,dim=1))
!*lq    integer      :: info            ! Must be of default integer kind
!*lq    integer      :: nh1, nh2        ! Must be of default integer kind
!*lq    
!*lq    nh1 = size(h,dim=1) ; nh2 = size(h,dim=2)
!*lq    call quad_zheev('V','U',nh1,h(1:nh1,1:nh2),nh1,e(:),work,size(work),rwork,info)
!*lq    if (info/=0) then
!*lq      write (out,"(' quad_zheev returned ',i8)") info
!*lq      stop 'lapack_quad_zheev - zheev failed'
!*lq    end if
!*lq  end subroutine lapack_quad_zheev
...
!*lq  end subroutine lapack_ginverse_quad
  ...
  subroutine lapack_ginverse_doublecomplex(amat,power_,eps_)
