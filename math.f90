...
!
  interface MathInterpolate
    module procedure MathInterpolateReal
    module procedure MathInterpolateComplex
  end interface MathInterpolate
!
...
!       jp = y**a * x**b * MathJacobiPn(n,real(a,kind=rk),real(b,kind=rk),x**2-y**2)
  ...
  !
  function MathInterpolateReal(x,xtab,vtab) result (v)
    ...
    !
    real(xk)    :: c(size(xtab)), d(size(xtab)), scl
    !
    include 'math_mathinterpolate_common.f90'
  end function MathInterpolateReal
    !
